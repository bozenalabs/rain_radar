...
void inky_sleep(int wake_in_minutes) {
    ...
    if(wake_in_minutes != -1) {
      inky_frame.rtc.set_timer(wake_in_minutes, PCF85063A::TIMER_TICK_1_OVER_60HZ);
      inky_frame.rtc.enable_timer_interrupt(true, false);
    }
...
int main()
{
    ...
    // inky_sleep(SLEEP_AFTER_UPDATE_MINUTES);


    inky_frame.sleep_until(0, 2, 0, 0);
